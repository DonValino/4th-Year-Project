...
class CancelRequestModel {
    ...
    function GetCancelRequestByUserIdANDTargetUserId($userId,$tagerUserId)
    {
        ...
        //Open connection and Select database
        $connection = mysqli_connect($host, $user, $passwd, $database);
        $result = mysqli_query($connection," SELECT * FROM cancelrequest WHERE userId=$userId AND tagerUserId='$tagerUserId' ORDER BY date DESC") or die(mysql_error());
